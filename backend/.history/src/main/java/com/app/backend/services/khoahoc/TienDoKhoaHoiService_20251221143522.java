...
public class TienDoKhoaHoiService implements ITienDoKhoaHoiService {
    ...
    @Override
    @Transactional
    public TienDoKhoaHoc getOrCreateProgress(Long userId, Long khoaHocId) {
    ...
    @Override
    @Transactional
    public void updateProgressAfterPractice(Long userId, Long boCauHoiId, Integer diemSo, BigDecimal doChinhXac) {
        ...
        boolean datDiemToiThieu = diemToiThieu == null || diemSo >= diemToiThieu;
        
        // Cập nhật trạng thái bộ câu hỏi
        if (datDiemToiThieu && !"HOAN_THANH".equals(tienDoBoCauHoi.getTrangThai())) {
            ...
            // Cập nhật số bộ đã hoàn thành trong khóa học (sau khi đã save)
            long soBoHoanThanh = tienDoBoCauHoiTrongKhoaRepository
                    .findCompletedBoCauHoi(tienDoKhoaHoc.getId()).size();
            ...
            // Tự động unlock bộ câu hỏi tiếp theo
            checkAndUnlockNextBoCauHoi(userId, khoaHocId, boCauHoiId);
        } else if (!"HOAN_THANH".equals(tienDoBoCauHoi.getTrangThai())) {
            // Nếu chưa hoàn thành nhưng đã bắt đầu luyện tập
            tienDoBoCauHoi.setTrangThai("DANG_HOC");
            tienDoBoCauHoiTrongKhoaRepository.save(tienDoBoCauHoi);
        }
        // Nếu đã HOAN_THANH thì đã save ở trên rồi, không cần save lại
    }

    @Override
    @Transactional
    public TienDoBoCauHoiTrongKhoa getOrCreateBoCauHoiProgress(Long tienDoKhoaHocId, Long boCauHoiId) {
    ...
    @Override
    @Transactional
    public void checkAndUnlockNextBoCauHoi(Long userId, Long khoaHocId, Long completedBoCauHoiId) {
        ...
        if (Boolean.TRUE.equals(nextBoCauHoi.getCanMoKhoa())
            ...
            if (!daMoKhoa) {
                // Tự động unlock (miễn phí vì đã hoàn thành bộ trước)
