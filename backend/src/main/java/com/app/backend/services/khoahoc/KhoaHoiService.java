...
public class KhoaHoiService implements IKhoaHoiService {
    ...
    public KhoaHoiDetailResponse getDetailById(Long id, Long currentUserId, boolean isAdmin) throws DataNotFoundException, PermissionDenyException {
        ...
        for (KhoaHocBoCauHoi khbch : danhSachBoCauHoi) {
            ...
            if (currentUserId != null) {
                ...
                if (isOwner || isAdmin) {
                    ...
                    if (boCauHoiId != null) {
                        ...
                        // Đếm số lượng câu hỏi đã trả lời sai (từ the_ghi_nho) - đếm DISTINCT câu hỏi
                        List<TheGhiNho> theGhiNhoList = theGhiNhoRepository.findAllByUserIdAndBoCauHoiId(currentUserId, boCauHoiId);
                        if (theGhiNhoList != null && !theGhiNhoList.isEmpty()) {
                            // Đếm số lượng câu hỏi DISTINCT (không phải số lượng the_ghi_nho)
                            soCauSai = (int) theGhiNhoList.stream()
            ...
            boCauHoiResponses.add(BoCauHoiTrongKhoaResponse.from(khbch, trangThai, daMoKhoa, soCauSai));
        }

        // Lấy tiến độ nếu user đã đăng nhập
        TienDoKhoaHoiResponse tienDo = null;
        Boolean daMoKhoaKhoaHoc = false;
        if (currentUserId != null) {
            ...
            // Kiểm tra unlock status của khóa học
            boolean isOwner = khoaHoc.getTaoBoi() != null && khoaHoc.getTaoBoi().getId().equals(currentUserId);
            if (isOwner || isAdmin) {
                daMoKhoaKhoaHoc = true;
            } else {
                daMoKhoaKhoaHoc = hasUserUnlockedKhoaHoc(id, currentUserId);
            }
        }
        ...
        return KhoaHoiDetailResponse.builder()
                ...
                .tienDo(tienDo)
                .daMoKhoaKhoaHoc(daMoKhoaKhoaHoc)
                .build();
    }
