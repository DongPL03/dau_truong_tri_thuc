...
public class SecurityConfig {
    ...
    public UserDetailsService userDetailsService() {
        return subject -> {
            ...
            Optional<NguoiDung> userByTenDangNhap = nguoiDungRepository.findNguoiDungByTenDangNhap(subject);
            if (userByTenDangNhap.isPresent()) {
                return userByTenDangNhap.get(); // Return UserDetails if found
            }

            // If user not found by phone number, attempt to find by email
            Optional<NguoiDung> userByEmail = nguoiDungRepository.findByEmail(subject);
