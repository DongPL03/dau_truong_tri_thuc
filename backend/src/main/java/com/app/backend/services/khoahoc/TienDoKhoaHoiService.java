...
public class TienDoKhoaHoiService implements ITienDoKhoaHoiService {
    ...
    public void updateProgressAfterPractice(Long userId, Long boCauHoiId, Integer diemSo, BigDecimal doChinhXac) throws DataNotFoundException {
        ...
        if (datDiemToiThieu) {
            ...
            boolean wasCompleted = "HOAN_THANH".equals(tienDoKhoaHoc.getTrangThai());
            if (tienDoKhoaHoc.getSoBoDaHoanThanh() >= tienDoKhoaHoc.getTongSoBo()) {
                if (!wasCompleted) {
                    // Lần đầu hoàn thành khóa học - xử lý achievement
                    tienDoKhoaHoc.setTrangThai("HOAN_THANH");
                    tienDoKhoaHoc.setNgayHoanThanh(Instant.now());
                    
                    // Xử lý achievement số lượng khóa học đã hoàn thành
                    thanhTichService.processAfterCourseComplete(userId);
                    
                    // Kiểm tra điểm cao (>= 90%) và unlock achievement
                    if (tienDoKhoaHoc.getPhanTramHoanThanh() != null 
                            && tienDoKhoaHoc.getPhanTramHoanThanh().compareTo(BigDecimal.valueOf(90)) >= 0) {
                        // Unlock achievement điểm cao thông qua processAfterCourseComplete
                        // (cần thêm logic vào ThanhTichService để check điểm cao)
                        thanhTichService.processAfterCourseComplete(userId);
                    }
                }
            }
            ...
            // Tự động unlock bộ câu hỏi tiếp theo khi đạt điểm tối thiểu
            // (unlock cả khi đã HOAN_THANH từ trước, để hỗ trợ practice từ memo)
    ...
    public void checkAndUnlockNextBoCauHoi(Long userId, Long khoaHocId, Long completedBoCauHoiId) throws DataNotFoundException {
        System.out.println("DEBUG checkAndUnlockNextBoCauHoi: userId=" + userId 
            + ", khoaHocId=" + khoaHocId + ", completedBoCauHoiId=" + completedBoCauHoiId);
        ...
        if (completedKhbch == null) {
            System.out.println("DEBUG checkAndUnlockNextBoCauHoi: Không tìm thấy bộ câu hỏi trong khóa học");
            return;
        }

        Integer thuTuHienTai = completedKhbch.getThuTu();
        System.out.println("DEBUG checkAndUnlockNextBoCauHoi: ThuTu hiện tại = " + thuTuHienTai);
        ...
        if (nextKhbch == null) {
            ...
            System.out.println("DEBUG checkAndUnlockNextBoCauHoi: Không còn bộ câu hỏi tiếp theo");
            return;
        }

        BoCauHoi nextBoCauHoi = nextKhbch.getBoCauHoi();
        System.out.println("DEBUG checkAndUnlockNextBoCauHoi: Tìm thấy bộ câu hỏi tiếp theo ID=" + nextBoCauHoi.getId());
        ...
        System.out.println("DEBUG checkAndUnlockNextBoCauHoi: daUnlockKhoaHoc = " + daUnlockKhoaHoc);
        
        if (!daUnlockKhoaHoc) {
            // User chưa unlock khóa học, không tự động unlock bộ câu hỏi tiếp theo
            System.out.println("DEBUG checkAndUnlockNextBoCauHoi: User chưa unlock khóa học, không unlock bộ tiếp theo");
            return;
        }
        ...
        if (Boolean.TRUE.equals(nextBoCauHoi.getCanMoKhoa())
            ...
            System.out.println("DEBUG checkAndUnlockNextBoCauHoi: Bộ tiếp theo cần unlock, daMoKhoa = " + daMoKhoa);

            if (!daMoKhoa) {
                // Tự động unlock (miễn phí vì đã unlock khóa học và hoàn thành bộ trước)
                System.out.println("DEBUG checkAndUnlockNextBoCauHoi: Tự động unlock bộ câu hỏi tiếp theo");
                ...
                System.out.println("DEBUG checkAndUnlockNextBoCauHoi: Đã unlock và cập nhật trạng thái thành công");
            } else {
                System.out.println("DEBUG checkAndUnlockNextBoCauHoi: Bộ câu hỏi tiếp theo đã được unlock rồi");
            }
        } else {
            System.out.println("DEBUG checkAndUnlockNextBoCauHoi: Bộ câu hỏi tiếp theo không cần unlock");
        }
    }
}
