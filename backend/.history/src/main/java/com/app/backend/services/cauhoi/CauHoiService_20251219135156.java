    ...
    public Page<CauHoi> findAll(Long boCauHoiId,
        ...
        if (boCauHoiId != null && boCauHoiId > 0) {
            ...
            if (boCauHoi != null) {
                ...
                // Nếu PRIVATE và không phải owner/admin -> chặn
                if (!isAdmin && !isOwner && Objects.equals(boCauHoi.getCheDoHienThi(), "PRIVATE")) {
                    throw new PermissionDenyException("Bộ câu hỏi này là riêng tư");
                }

                // Admin và Owner bypass unlock check (PUBLIC)
                if (!isAdmin && !isOwner && Objects.equals(boCauHoi.getCheDoHienThi(), "PUBLIC")) {
                    // Nếu không xác định được user hiện tại (dù đã auth) thì chặn
                    if (creatorId == null) {
                        throw new PermissionDenyException("Bạn cần đăng nhập để xem danh sách câu hỏi");
                    }
