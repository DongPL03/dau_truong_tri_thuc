...
export class BoCauHoiList extends Base implements OnInit {
  ...
  goToCreateQuiz() {
    ...
    this.router.navigateByUrl('/bo-cau-hoi/create').then(r => {
    });
  }
  ...
  navigateDetail(id: number) {
    this.router.navigate(['/bo-cau-hoi/detail', id]).then(r => {
    });
  }
  ...
  navigateEdit(id: number) {
    this.router.navigate(['/bo-cau-hoi/edit', id]).then(r => {
    });
  }
}
