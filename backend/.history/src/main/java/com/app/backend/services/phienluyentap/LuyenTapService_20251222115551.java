...
public class LuyenTapService implements ILuyenTapService {
    ...
    public SubmitLuyenTapResponse guiDapAn(TraLoiCauHoiRequestDTO request, Long userId)
        ...
        // Kiểm tra xem có phải practice từ memo không
        // Practice từ memo khi: tongCauHoi <= số lượng the_ghi_nho hiện tại
        // và tongCauHoi <= tổng số câu hỏi trong bộ
        BoCauHoi boCauHoi = phien.getBoCauHoi();
        boolean isPracticeFromMemo = false;
        if (boCauHoi != null && phien.getTongCauHoi() != null) {
            // Lấy số lượng the_ghi_nho hiện tại của user cho bộ này
            List<TheGhiNho> currentMemos = theGhiNhoRepository.findAllByUserIdAndBoCauHoiId(userId, boCauHoi.getId());
            int soLuongTheGhiNho = currentMemos != null ? currentMemos.size() : 0;

            // Practice từ memo khi:
            // 1. tongCauHoi <= số lượng the_ghi_nho (vì practice từ memo chỉ lấy các câu trong the_ghi_nho)
            // 2. VÀ có ít nhất 1 the_ghi_nho
            // 3. VÀ tongCauHoi <= soCauHoi (để phân biệt với practice bình thường)
            if (soLuongTheGhiNho > 0
        ...
        for (TraLoiCauHoiRequestDTO.CauTraLoiRequest ans : request.getCauTraLoiList()) {
            ...
            if (isPracticeFromMemo) {
                // Khi practice từ memo:
                if (correct) {
                    // Nếu trả lời đúng, xóa the_ghi_nho của câu hỏi này
                ...
                // Nếu trả lời sai trong practice từ memo, KHÔNG thêm vào the_ghi_nho nữa
                // (vì đã có sẵn rồi từ lần practice trước)
            } else {
                // Khi practice bình thường (không phải từ memo):
                if (!correct && !theGhiNhoRepository.existsByPhienIdAndCauHoiId(phien.getId(), cauHoi.getId())) {
                    // Kiểm tra xem đã có trong the_ghi_nho của bộ câu hỏi này chưa
                    List<TheGhiNho> existingMemos = theGhiNhoRepository.findAllByUserIdAndBoCauHoiId(
                            userId, boCauHoi.getId());
                    boolean alreadyExists = existingMemos.stream()
                            .anyMatch(m -> m.getCauHoi().getId().equals(cauHoi.getId()));

                    if (!alreadyExists) {
                        // Lưu vào the_ghi_nho nếu trả lời sai và chưa có trong the_ghi_nho
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<PhienLuyenTap> getPracticeHistory(Long userId, boolean isAdmin, Long khoaHocId, Long boCauHoiId, PageRequest pageRequest) {
        if (isAdmin) {
            // Admin có thể xem tất cả, nhưng vẫn có thể filter
            return phienLuyenTapRepository.findByFilters(null, khoaHocId, boCauHoiId, pageRequest);
        } else {
            return phienLuyenTapRepository.findByFilters(userId, khoaHocId, boCauHoiId, pageRequest);
        }
    }
