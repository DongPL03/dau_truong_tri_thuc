...
export class BoCauHoiDetail extends Base implements OnInit {
  ...
  goCreateQuestion() {
    this.router.navigate(['/bo-cau-hoi', this.boCauHoiId, 'questions', 'new']).then(r => {
    });
  }

  getMediaUrl(q: CauHoiResponse): string {
    if (!q.duong_dan_tep) return '';
    return `http://localhost:8088/api/v1/cauHoi/media/${q.duong_dan_tep}`;
  }

  navigateToEdit(id: number): void {
    this.router.navigateByUrl(`/bo-cau-hoi/${this.boCauHoiId}/cau-hoi/${id}/edit`).then(r => {
    });
  }
