...
public class BoCauHoiService implements IBoCauHoiService {
    ...
    public BoCauHoi create(BoCauHoiDTO boCauHoiDTO, Long currentUserId) throws DataNotFoundException, PermissionDenyException {
        ...
        BoCauHoi boCauHoi = BoCauHoi.builder()
                ...
                .isXoa(false)
                .taoBoi(taoBoi)
                .canMoKhoa(false) // mặc định không cần mở khóa, admin sẽ set khi duyệt nếu muốn thu phí
                .giaMoKhoa(0L)
    ...
    public BoCauHoi approve(Long id, Long adminId) {
        ...
        // Gợi ý giá mở khóa dựa trên số lượng câu hỏi (admin có thể chỉnh sau này)
    ...
    public UnlockBoCauHoiResponse unlockBoCauHoi(Long boCauHoiId, Long userId) throws Exception {
        ...
        // Chia thưởng cho creator (70%) nếu không phải admin và không phải tự mở khoá của mình
        if (bo.getTaoBoi() != null
                && !bo.getTaoBoi().getId().equals(userId)
                && !securityUtils.isAdmin()) {
            long reward = Math.round(price * 0.7);
            Long creatorId = bo.getTaoBoi().getId();

            BangXepHang creatorBx = bangXepHangRepository.findByNguoiDung_Id(creatorId)
                    .orElseThrow(() -> new DataNotFoundException("Không tìm thấy bảng xếp hạng của tác giả bộ câu hỏi"));
